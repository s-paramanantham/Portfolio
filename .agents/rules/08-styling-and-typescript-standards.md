...
## 1. Mandatory Tailwind CSS Styling & Responsive Design
...
- **Mandatory Mobile + Web Responsive UI**:
  - Mobile (<640px): 1-column layouts, touch targets >= 44x44px, hamburger navigation.
  - Tablet (640-1024px): 2-column layouts.
  - Laptop / Desktop (>1024px): Multi-column layouts, rich hover effects.
...
- **Dark Mode**: Dark-first palette (`bg-[#030712]`, `text-slate-100`, `border-slate-800/80`, `backdrop-blur-md`).
...
- **Proper Alternatives**:
  ...
  - Props & ViewModel State:
    ...
    export interface UseUserProfileViewModelReturn {
      ...
      readonly handleUpdate: (updated: UserProfileBo) => Promise<void>;
    }
    ```
  - Generic API Requests:
    ...
    ```
- **Validation**:
  - Run `npm run typecheck` to verify 100% type safety with zero errors.
