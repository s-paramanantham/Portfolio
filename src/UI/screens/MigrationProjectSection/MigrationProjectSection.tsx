  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-10 sm:mb-16 space-y-2.5 sm:space-y-3">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-300 dark:border-indigo-500/30 text-indigo-800 dark:text-indigo-300 text-xs font-semibold uppercase tracking-wider">
        ...
        <p className="text-slate-600 dark:text-slate-400 text-xs sm:text-sm md:text-base max-w-3xl mx-auto leading-relaxed">
          Production-grade enterprise migration solution with 200+ Node.js/Express REST APIs, automated inventory discovery, Slack corporate export ingestion, and OneDrive/SharePoint uploads engineered at <strong className="text-slate-900 dark:text-white">AVASOFT</strong>.
        </p>
      </div>
      ...
      <GlassSurface intensity="high" borderGlow className="p-4 sm:p-6 lg:p-10 space-y-6 sm:space-y-8 w-full overflow-hidden">
        ...
        <div>
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-3">
            ...
            <Button
              ...
              leftIcon={
                isSimulatingMigration ? (
                  <RotateCcw className="w-3.5 h-3.5 animate-spin text-cyan-600 dark:text-cyan-400" />
                ) : (
          ...
          <div className="space-y-2 mb-6">
            <div className="w-full bg-slate-200 dark:bg-slate-950/80 rounded-full h-2.5 border border-slate-300 dark:border-slate-800 overflow-hidden">
              <div
            ...
            <div className="flex items-center justify-between text-xs font-mono text-slate-500 dark:text-slate-400">
              <span className="flex items-center gap-2 truncate pr-2 text-cyan-700 dark:text-cyan-300 font-medium">
                <span className={`w-2 h-2 rounded-full shrink-0 ${isSimulatingMigration ? 'bg-cyan-500 animate-ping' : 'bg-emerald-500'}`} />
                <span className="truncate">{simulationStatus}</span>
              </span>
              <span className="shrink-0 font-bold text-slate-700 dark:text-slate-300">{migrationProgress}%</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div
              ...
              className={`p-4 sm:p-5 rounded-xl border transition-all duration-300 cursor-pointer ${
                activeStage === 'source'
                  ? 'bg-cyan-50/80 dark:bg-slate-800/80 border-cyan-400 dark:border-cyan-400 shadow-md shadow-cyan-500/10'
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-mono uppercase text-slate-500 dark:text-slate-400">Step 1: Ingestion</span>
                <Badge variant="cyan" size="sm">Slack Enterprise API</Badge>
              </div>
              <h4 className="text-base font-bold text-slate-900 dark:text-white">Slack Workspace Ingestion</h4>
            ...
            <div
              ...
              className={`p-4 sm:p-5 rounded-xl border transition-all duration-300 cursor-pointer ${
                activeStage === 'processing'
                  ? 'bg-indigo-50/80 dark:bg-slate-800/80 border-indigo-400 dark:border-indigo-400 shadow-md shadow-indigo-500/10'
                  : 'bg-slate-50 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700'
              }`}
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-mono uppercase text-slate-500 dark:text-slate-400">Step 2: Engine</span>
                <Badge variant="indigo" size="sm">200+ Node/Express APIs</Badge>
              </div>
              <h4 className="text-base font-bold text-slate-900 dark:text-white">Transformation &amp; SQL Staging</h4>
              <p className="text-xs text-slate-600 dark:text-slate-400 mt-2 leading-relaxed">
            ...
            <div
              ...
              className={`p-4 sm:p-5 rounded-xl border transition-all duration-300 cursor-pointer ${
                activeStage === 'destination'
                  ? 'bg-emerald-50/80 dark:bg-slate-800/80 border-emerald-400 dark:border-emerald-400 shadow-md shadow-emerald-500/10'
                  : 'bg-slate-50 dark:bg-slate-900/40 border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700'
              }`}
            >
              <div className="flex items-center justify-between mb-3">
                <span className="text-xs font-mono uppercase text-slate-500 dark:text-slate-400">Step 3: Destination</span>
                <Badge variant="emerald" size="sm">Teams &amp; OneDrive</Badge>
              </div>
              <h4 className="text-base font-bold text-slate-900 dark:text-white">Teams Channel &amp; File Ingestion</h4>
              <p className="text-xs text-slate-600 dark:text-slate-400 mt-2 leading-relaxed">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-slate-200 dark:border-slate-800">
          <div className="p-3 sm:p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex flex-col justify-center">
            <span className="text-2xl sm:text-3xl font-mono font-bold text-indigo-600 dark:text-indigo-400">
              <AnimatedCounter value={200} suffix="+" />
            </span>
            <span className="text-xs text-slate-800 dark:text-slate-300 font-semibold mt-1">APIs Developed</span>
            <span className="text-[11px] text-slate-500 mt-0.5">Node.js &amp; Express.js</span>
          </div>

          <div className="p-3 sm:p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex flex-col justify-center">
            <span className="text-2xl sm:text-3xl font-mono font-bold text-cyan-600 dark:text-cyan-400">
              <AnimatedCounter value={500} suffix="+" />
            </span>
            <span className="text-xs text-slate-800 dark:text-slate-300 font-semibold mt-1">Users Supported</span>
          ...
          <div className="p-3 sm:p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex flex-col justify-center">
            ...
            <span className="text-xs text-slate-800 dark:text-slate-300 font-semibold mt-1">Chats Migrated</span>
          ...
          <div className="p-3 sm:p-4 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 flex flex-col justify-center">
            ...
            <span className="text-xs text-slate-800 dark:text-slate-300 font-semibold mt-1">Data Volume</span>
            <span className="text-[11px] text-slate-500 mt-0.5">0 Throttling Incidents</span>
          </div>
        </div>

        {/* Technical Highlights Breakdown */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 pt-4 sm:pt-6 border-t border-slate-200 dark:border-slate-800">
          <div className="space-y-3">
            <h4 className="text-xs font-semibold text-slate-800 dark:text-slate-300 uppercase tracking-wider flex items-center gap-2">
              <ShieldCheck className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
              Super Admin Management &amp; Rate-Limiting
            </h4>
            <ul className="space-y-2 text-xs text-slate-700 dark:text-slate-300">
              <li className="flex items-start gap-2">
                <KeyRound className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400 shrink-0 mt-0.5" />
                <span>Client ID &amp; Client Secrets authentication with organization inventory validation.</span>
              </li>
              <li className="flex items-start gap-2">
                <CalendarClock className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400 shrink-0 mt-0.5" />
